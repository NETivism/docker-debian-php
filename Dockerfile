...
RUN \
  ...
  bash -c "echo extension=/usr/lib/php/20180731/mcrypt.so > /etc/php/7.4/mods-available/mcrypt.ini" && \
...
RUN \
  ...
  pecl install xdebug && \
  bash -c "echo zend_extension=/usr/lib/php/20180731/xdebug.so > /etc/php/7.4/mods-available/xdebug.ini" && \
